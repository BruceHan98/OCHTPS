...
def check_and_validate_polys(polys, size):
    """
...
def augmentation(im: np.ndarray, text_polys: np.ndarray, text_label, text_lengths, label_strs, epoch_count) -> tuple:
    ...
    if len(text_polys) >= 4:
        ...
        if choice_pos < 0.5:
            fill_box = np.int0(
                [text_polys[mid1 - 1, 3], text_polys[mid1 - 1, 2], [im.shape[1], im.shape[0]], [0, im.shape[0]]])
...
def augmentation1(im: np.ndarray, text_polys: np.ndarray, text_label, text_lengths, label_strs, epoch_count) -> tuple:
    ...
    if len(text_polys) >= 3:
        ...
        if epoch_count % 3 == 0:
            ...
            fill_box2 = np.int0(
                [text_polys[mid2 - 1, 3], text_polys[mid2 - 1, 2], [im.shape[1], im.shape[0]], [0, im.shape[0]]])
...
def image_label(im_fn: str, text_polys: np.ndarray, text_label: list, text_lengths, label_strs, n: int, m: float,
                input_size: int,
                defrees: int = 10,
    ...
    return imgs[0], imgs[1].transpose((2, 0, 1)), imgs[
        2], text_polys, text_label, text_lengths, label_strs  # im,score_maps,training_mask#


class MyDataset(data.Dataset):
    def __init__(self, data_dirs, char_dict, data_shape: int = 640, n=6, m=0.5, transform=None, target_transform=None,
                 max_text_length=80,
